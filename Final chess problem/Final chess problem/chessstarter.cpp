...
double eye[3] = { 9000, 4000, 10000 }; // pick a nice vantage point.
...
void animate(){
	...
	double pX;
	if (Time >= 1 && Time <= 3){
		//rook is pieces 31 and 23
		Scale scale;
		double scaleX, scaleZ = 0.0;
		RatioSet(Time, 1, 3, scaleX, 1.0, 2.0);
		RatioSet(Time, 1, 3, scaleZ, 1.0, 2.0);
		scale.x = scaleX;
		scale.y = 1.0;
		scale.z = scaleZ;
		pieces[24].setScale(scale);
		pieces[31].setScale(scale);
	}
	if (Time >= 4 && Time <= 6){
		double rotation = 0;
		RatioSet(Time, 4, 6, rotation, 0.0, 180);
		pieces[27].setRotation(rotation, 0, 0);
	}
}
//
