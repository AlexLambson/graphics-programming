...
void Maze::Cell::Draw(int mazeX, int mazeY, GLuint * texName){
	...
	if (Globals::current_view == perspective_view || Globals::current_view == rat_view){
		...
		if (this->walls.top && this->position.yPos == mazeY - 1) {
			glTexCoord2d(0, y1); glVertex3f(x, y + 1, 0);
			glTexCoord2d(1, y1); glVertex3f(x + 1, y + 1, 0);
			glTexCoord2d(1, y2); glVertex3f(x + 1, y + 1, 1);
			glTexCoord2d(0, y2); glVertex3f(x, y + 1, 1);
		}
		if (this->walls.bottom) {
			glTexCoord2d(0, y1); glVertex3f(x, y, 0);
			glTexCoord2d(1, y1); glVertex3f(x + 1, y, 0);
			glTexCoord2d(1, y2); glVertex3f(x + 1, y, 1);
			glTexCoord2d(0, y2); glVertex3f(x, y, 1);
		}
		if (this->walls.right && this->position.xPos == mazeX - 1) {
			glTexCoord2d(0, y1); glVertex3f(x + 1, y + 1, 0);
			glTexCoord2d(1, y1); glVertex3f(x + 1, y, 0);
			glTexCoord2d(1, y2); glVertex3f(x + 1, y, 1);
			glTexCoord2d(0, y2); glVertex3f(x + 1, y + 1, 1);
		}
		if (this->walls.left) {
			glTexCoord2d(0, y1); glVertex3f(x, y, 0);
			glTexCoord2d(1, y1); glVertex3f(x, y + 1, 0);
			glTexCoord2d(1, y2); glVertex3f(x, y + 1, 1);
			glTexCoord2d(0, y2); glVertex3f(x, y, 1);
		}
		glEnd(); 
		glDisable(GL_TEXTURE_2D);
