...
bool Rat::CanMove(double dx, double dy, CellWalls walls){
	...
	if (offsetX - this->mSize / 2 < -0.00004 && walls.left){
		//return false;
		gMaze.mCells[xIndex][yIndex].walls.left = false;
		if (xIndex >= 1) gMaze.mCells[xIndex - 1][yIndex].walls.right = false;
	}
	if (offsetX + this->mSize / 2 > 1.00004 && walls.right){
		//return false;
		gMaze.mCells[xIndex][yIndex].walls.right = false;
		if (xIndex < gMaze.getMazeSize().sizeX - 1) gMaze.mCells[xIndex + 1][yIndex].walls.left = false;
	}
	if (offsetY - this->mSize / 2 < 0 && walls.bottom){
		//return false;
		gMaze.mCells[xIndex][yIndex].walls.bottom = false;
		if (yIndex >= 1) gMaze.mCells[xIndex][yIndex - 1].walls.top = false;
	}
	if (offsetY + this->mSize / 2 > 1 && walls.top){
		//return false;
		gMaze.mCells[xIndex][yIndex].walls.top = false;
		if (yIndex < gMaze.getMazeSize().sizeY - 1) gMaze.mCells[xIndex][yIndex + 1].walls.bottom = false;
	}
	return true;
}
...
void Rat::Move(CellWalls walls, Key::Type key){
	...
	if (key == Key::UP || key == Key::DOWN){
		newX = dx + this->mPosition.xPos;
		newY = dy + this->mPosition.yPos;
	}
	else if (key == Key::Q){
		newX = (dy * -1.0) + this->mPosition.xPos;
		newY = dx + this->mPosition.yPos;
	}
	else if (key == Key::E){
		newX = dy + this->mPosition.xPos;
		newY = (dx * -1.0) + this->mPosition.yPos;
	}
